...
- [X] T003 Agregar dependencia `league/csv` en `backend/composer.json` y ejecutar `docker compose exec backend composer install`
...
- [X] T005 [P] Crear modelo `Evaluation` en `backend/app/Models/Evaluation.php` con:
  ...
  - `$guarded`: id
- [X] T006 [P] Crear directorio `backend/storage/app/evaluations/.gitkeep` para exportaciones
- [X] T007 Ejecutar migración: `docker compose exec backend php artisan migrate`
...
- [X] T008 [P] [US1] Crear `MetricsCalculatorService` en `backend/app/Services/MetricsCalculatorService.php` con métodos públicos:
...
- [X] T009 [P] [US1] Crear `MeasurementService` en `backend/app/Services/MeasurementService.php` como **orquestador exclusivamente** — NO debe contener lógica de cálculo, detección, renderizado ni exportación. Solo orquesta servicios especializados inyectados vía constructor. Método:
...
- [X] T010 [US1] Implementar manejo de casos borde en `MetricsCalculatorService`:
...
- [X] T011 [P] [US5] Crear `MetricsExporter` en `backend/app/Exports/MetricsExporter.php` con métodos:
...
- [X] T012 [P] [US5] Crear `EvaluationController` en `backend/app/Http/Controllers/EvaluationController.php` con:
...
- [X] T013 [P] [US5] Crear `EvaluationResource` en `backend/app/Http/Resources/EvaluationResource.php` que estructura la respuesta JSON según el contrato (route_metrics, anomalies, ranking, metrics_summary, files)

- [X] T014 [US5] Registrar rutas en `backend/routes/api.php`:
  ```php
...
- [X] T015 [US5] Implementar validación de parámetros en `EvaluationController@store`:
...
- [X] T016 [P] [US2] Crear `AnomalyDetector` en `backend/app/Services/AnomalyDetector.php` con:
...
- [X] T017 [US2] Integrar `AnomalyDetector` en `MeasurementService::execute()`:
...
- [X] T018 [P] [US4] Crear `MapRendererService` en `backend/app/Services/MapRendererService.php` con:
...
- [X] T019 [US4] Integrar `MapRendererService` en `MeasurementService::execute()`:
...
- [X] T020 [P] [US3] Extender `GET /api/evaluations` en `EvaluationController` para incluir en el listado todos los campos de `metrics_summary` necesarios para comparación rápida: coverage_territorial_km, distancia_promedio_general_km, balance_index, inter_cluster_min_distance_km, operational_penalty_total, total_anomalias_detectadas
- [X] T021 [US3] Extender `GET /api/evaluations/{id}` para incluir el ranking de rutas en la respuesta (mismos campos que POST según contrato)
...
- [X] T022 [P] Verificar casos borde en `MetricsCalculatorService`:
  - Ruta con 0 paquetes: métricas en cero, centroide = bodega ✅
  - Rutas pero 0 entregas asignadas: métricas globales en cero ✅
  - 1 sola ruta: separación entre clusters = 0 ✅
- [X] T023 [P] Verificar exportación: JSON (42KB), CSV (559B), deliveries.csv (6KB) válidos ✅
- [X] T024 [P] Verificar mapas: 7 PNG generados, bounding box contiene todos los puntos, colores distintos por ruta ✅
- [X] T025 [P] Verificar reproducibilidad: dos ejecuciones con mismos parámetros producen métricas idénticas ✅
- [X] T026 Ejecutar y verificar todos los escenarios de `quickstart.md` en `specs/003-results-measurement/quickstart.md`
